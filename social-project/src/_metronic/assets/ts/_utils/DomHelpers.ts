...
function getCSSVariableValue(variableName: string) {
  let hex = getComputedStyle(window.document.documentElement).getPropertyValue(variableName)
  if (hex && hex.length > 0) {
    hex = hex.trim()
  }
...
function getElementParents(element: Element, selector: string) {
  ...
  if (!Element.prototype.matches) {
    Element.prototype.matches = function (s) {
      const matches = (typeof window !== 'undefined' && window.document || this.ownerDocument).querySelectorAll(s)
  ...
  for (;typeof window !== 'undefined' && el && el !== window.document.body; el = el.parentElement) {
    if (selector) {
...
function getHighestZindex(el: HTMLElement) {
  ...
  let buffer = el
  while (bufferNode && typeof window !== 'undefined' && bufferNode !==  window.document) {
...
function getScrollTop(): number {
  return (typeof window !== 'undefined' && window.document.scrollingElement || window.document.documentElement).scrollTop
}
