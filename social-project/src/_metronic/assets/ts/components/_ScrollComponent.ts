...
class ScrollComponent {
  ...
  private getAutoHeight = () => {
    ...
    if (dependencies !== null) {
      const elements = window.document.querySelectorAll(dependencies as string)
    ...
    // Wrappers
    if (wrappers !== null) {
      const elements = window.document.querySelectorAll(wrappers as string)
      if (elements && elements.length > 0) {
        for (let i = 0, len = elements.length; i < len; i++) {
          const element = elements[i] as HTMLElement
  ...
  public static updateAll() {
    const elements = window.document.body.querySelectorAll('[data-kt-scroll="true"]')
    elements.forEach((element: Element) => {
