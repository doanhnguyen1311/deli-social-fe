...
class MenuComponent {
  ...
  private _setTriggerElement = () => {
    const selector = this.options.triggerSelector || '[data-kt-menu-trigger]';
  ...
  _getCss = (el: HTMLElement, styleProp: string) => {
    const defaultView = (el.ownerDocument || typeof window !== 'undefined' && window.document).defaultView
    if (!defaultView) {
      return ''
    }
  ...
  _showDropdown = (item: HTMLElement) => {
    ...
    // Append the sub the the root of the menu
    {
      if (this._getItemOption(item, 'overflow') === true) {
        if (sub) {
          window.document.body.appendChild(sub)
    ...
    const attach = this._getItemOption(item, 'attach') as string
    {
      if (attach) {
        if (attach === 'parent') {
          ...
          reference = window.document.querySelector(attach)
        }
      } else {
        reference = item
      }
  ...
  public static hideDropdowns = (skip: HTMLElement | undefined) => {
    {
      const items = window.document.querySelectorAll<HTMLElement>(
  ...
  public static updateDropdowns = () => {
    {
      const items = window.document.querySelectorAll('.show.menu-dropdown[data-kt-menu-trigger]')
      if (items && items.length > 0) {
        for (let i = 0, len = items.length; i < len; i++) {
          const item = items[i]
  ...
  public static initGlobalHandlers = () => {
    if (this.triggerElement) {
      ...
      window.document.addEventListener('click', (e) => {
        const menuItems = window.document.querySelectorAll(
      ...
      window.addEventListener('resize', () => {
        ...
        throttle(
          ...
          () => {
            ...
            const elements = window.document.querySelectorAll('[data-kt-menu="true"]')
            elements.forEach((el) => {
