...
class SwapperComponent {
  ...
  public update = () => {
    if (typeof window !== 'undefined') {
      ...
      const parentElement = parentSelector ? window.document.querySelector(parentSelector) : null
      if (parentElement && this.element.parentNode !== parentElement) {
        const alreadyPended = window.document.getElementById('kt_header_menu') !== null
        if (!alreadyPended) {
          if (mode === 'prepend') {
    ...
    if (typeof window !== 'undefined') {
      throttle(
        ...
        () => {
          ...
          const elements = window.document.querySelectorAll(defaultSwapperQueires.instanseQuery)
          elements.forEach((el) => {
