...
class ThemeMode {
  ...
  private getParamName = (postfix: string): string | '' => {
    if (typeof window !== 'undefined') {
      const ktName = window.document.body.hasAttribute('data-kt-name')
  ...
  public getMode = (): Mode => {
    ...
    if (typeof window !== 'undefined' && !window.localStorage.getItem(modeParam)) {
      return defaultMode
    }

    const ls = typeof window !== 'undefined' && window.localStorage.getItem(modeParam)
    if (ls) {
      return ls as Mode
    }
  ...
  public setMode = (mode: Mode, menuMode: Mode | ''): void => {
    ...
    if (activeMenuItem && typeof window !== 'undefined' && window.localStorage) {
  ...
  public getMenuMode = (): Mode | '' => {
    ...
    const ls = typeof window !== 'undefined' && window.localStorage ? localStorage.getItem(menuModeParam) : null
    return (ls as Mode) || ''
  }
  ...
  private setActiveMenuItem = (item: HTMLElement): void => {
    ...
    if (typeof window !== 'undefined' && window.localStorage && menuMode && menuModeParam) {
      localStorage.setItem(menuModeParam, menuMode)
    }
  }
  ...
  public flipImages = () => {
    {
      window.document.querySelectorAll<HTMLElement>('[data-kt-img-dark]')?.forEach((item: HTMLElement) => {
        if (item.tagName === 'IMG') {
  ...
  public init = () => {
    {
      this.menu = window.document.querySelector<HTMLElement>('[data-kt-element="theme-mode-menu"]')
...
// Initialize app on window.document ready => ThemeModeComponent.init()
